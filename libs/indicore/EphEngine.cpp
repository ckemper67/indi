...
public:
    ...
    void GetPlanetTopocentric(int np, double jd, INDI::AstrometricContext &ctx,
        ...
        double rast, dast, rapp, dapp, eo, diam;
        ephPLANctx* pctx = (np == 3) ? m_cemb.get() : m_cplan.get();
        ephRdplan(m_cmoon.get(), m_cemb.get(), pctx, ut1_mjd, tdb_mjd, np,
                  DEG_TO_RAD(ctx.observer.longitude),
                  DEG_TO_RAD(ctx.observer.latitude),
                  ctx.observer.elevation,
                  &rast, &dast, &rapp, &dapp, &eo, &diam);
        out->rightascension = RAD_TO_HOURS(eraAnp(rapp));
        out->declination    = RAD_TO_DEG(dapp);
#else
        INDI_UNUSED(np); INDI_UNUSED(jd); INDI_UNUSED(ctx); INDI_UNUSED(out);
#endif
    }
