...
// ---------------------------------------------------------------------------
// SPK polar-degeneracy regression tests
//
    ...
    double fixedJD = 2461113.81667)
{
    ...
    else
    {
        ...
        pSupport->LocalHourAngleDeclinationFromTelescopeDirectionVector(resultV, eq);
        double encoderRA = range24(lst - eq.rightascension);
        ...
        // Tolerances are generous (10 deg) because this test uses large mount
        // errors (IH=5 deg, ID=5 deg) with a single sync point.  The goal is
        // to detect catastrophic failures (scope below horizon), not to verify
        // sub-degree accuracy.
        EXPECT_LT(std::abs(dRA), 10.0)
            << "EQ polar degeneracy: encoder RA=" << encoderRA
            << "h vs target RA=" << testRA
            << "h (dRA=" << dRA << " deg)"
            << " -- forward transform produced garbage encoder HA";
        EXPECT_LT(std::abs(eq.declination - testDec), 10.0)
